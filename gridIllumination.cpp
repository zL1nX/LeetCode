...
vector<int> gridIllumination(int n, vector<vector<int>>& lamps, vector<vector<int>>& queries) 
{
    ...
    unordered_set<pair<int, int>, decltype(hash_p)> points(0, hash_p); // 存储方式重载
    for(auto& lamp : lamps)
    {
        ...
        row[lamp[0]]++;
        col[lamp[1]]++;
        diagonal[lamp[0] - lamp[1]]++; // 对角线用这种方法来表示，学到了
    ...
    for(int i = 0;i < queries.size(); i++)
    {
        ...
        if(row.count(r) > 0 && row[r] > 0) // 只要有一个被照亮，就算是被照亮
        {
            res[i] =  1;
        }
        ...
        for(int x = r - 1; x <= r + 1; x++) // 这个写法挺不错的，即遍历一个网格周围8个方向
        {

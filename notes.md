...
- [698-划分为k个相等的子集](./canPartitionKSubsets.cpp), (相隔时间太长, 有点忘了)
  - 可以遍历球, 也可以遍历桶
  - 通过used与state来剪枝, 减少选择

- [22-括号生成](./generateParenthesis.cpp), 将左右括号**分别维护变量**去回溯, 这样结束条件和回溯的递归就都清楚了
  - 回溯的结束条件: left == right == 0, 即左右括号都用完了; 如果右括号数目比左括号少, 说明回溯路径是错的 (必然不合法)
  - 递归转移时, 其实就是backtrack遍历下选择, 这个题里就是依次选择左右两个括号进行下一次尝试即可

### 排列组合子集

- **元素不重且不可复选:** 
  - [78-子集](./subsets_I.cpp). 标准的回溯模板题. 使用一个全局变量`track`来记录回溯过程中的所有选择
    - 在backtrack内, 通过一循环来控制要把哪些元素加入到当前的track中.
    - 循环时从start开始, 在pop back结束回退到上一次递归时, 就可以继续在循环中添加其他所有元素
    - 本质上就是在模拟前序遍历树的过程 (就是最朴素的那种决策树, 按照前序顺序遍历)
  - [77-组合](./combineI.cpp), 在上一题的基础上, 只返回特定大小的子集元素
    - 因此, 只需要在之前代码的基础上, 在往`res`中添加`track`时, 当track的大小等于k时才被加入res中
    - 其他都完全一样, 同样是最朴素的决策树, 只不过要注意backtrack传参稍微有一点不一样
  - [46-全排列](./permute2.cpp), 之前回溯部分刚介绍的模板题. 不过根据前面两题的特点, 这一题依然可以沿用相似的模板
    - 但是需要增加一个used数组来跳过那些已经被选过的元素
    - 除此之外, 当track的长度等于nums时, 说明加完了一种结果, 就可以添加到res中

- **元素可重且不可复选**
  - [90-子集 II](./subsets_II.cpp), 数组中的元素可以重复, 那么如果还用之前的代码, 就会选择出重复的子集
    - 一种解决方法是, 先将数组排序 , 然后在backtrack的for循环挑选元素时, 跳过相同的元素 `nums[i] == nums[i - 1]`
    - 这样选出来的子集就没有重复的结果了
  - [40-组合总和 II](./combinationSum2.cpp), 和上一题一样, 唯一的区别在于, 需要选出和为`target`的子集
    - 因此在遍历时, 还要维护一个全局`trackSum`变量来记录每种track的大小, 当遇到相等的就加入res, 一旦大于就立即return
    - 注意在backtrack递归时, `trackSum`也要同步变化
  - [47-全排列 II](./permuteUnique.cpp), 这道题对于剪枝的处理很妙, 核心是要保证: **若干个相同元素如果要同时出现, 需要保证同一种相对顺序**, 否则就会出现重复排列
    - 先排序让相同元素紧邻在一起, 之后采取和上面两题类似的处理方法, 但有一点不同: 在挑选子集时, 元素可以被丢弃, 即可以直接跳过; 而在挑选排列时, 所有元素都要选上
    - 因此, 在本题中相同元素是可以被选择的, 但是必须要保持相对顺序不变 (类似于**一个整体**), 因此最粗暴的顺序就是直接按照sort后的顺序, 在保证前面元素被用过了后, 自己才能被选
      - `i > 0 && nums[i] == nums[i - 1] && !used[i - 1]`

- **元素无重且可以复选**
  - [39-组合总和](./combinationSum.cpp), 与选特定子集唯一的不同在于, 元素可以复选, 因此对比后可知:
    - 在backtrack递归时, 只要传入的位置参数依然是`i`而不是`i+1`即可, 这样在下一层回溯时, 依然可以再选一遍当前元素
    - 其他地方与[40-组合总和 II](./combinationSum2.cpp)都基本一致


## 岛屿问题

- **岛屿计数**
  - [200-岛屿数量](./numIslands_I.cpp), 岛屿问题的模板题, 通过dfs + direction数组 + 剪枝实现Flood and Fill
    - 循环时遇到一个岛屿, 就进入dfs, 将与其相连的**陆地**都置为海水, 即都淹掉即可. 这样在上层函数每遇到一个陆地, 都将是一个新岛屿区域的陆地, count + 1
    - 注意在dfs过程中, 遇到地方本身是0的了, 就直接return剪枝, 否则会爆栈
  - [1254-统计封闭岛屿的数目](./closedIsland.cpp), 与上一题基本一致但统计的是*封闭岛屿*的数量
    - 先通过dfs把**那些靠边的岛屿给淹**了, 剩下的就是岛屿就都是被海水包起来的了 (逆向思维)
    - 注意这一题海水跟陆地的符号反了, 而且是int (上一题是char)
  - [1020-飞地的数量](./numEnclaves.cpp), 与上一题的思路一致, 但计数的不是封闭岛屿, 而是封闭岛屿单元格的**数目**
    - 因此在主循环里就不用继续dfs了, 而是在淹完边界岛屿之后, 凡是单元格为1的, 就都计数
    - 注意在dfs循环里要**先进行边界检查**
  - [695-岛屿的最大面积](./maxAreaOfIsland.cpp), 同理套模板, 无非dfs可以返回结果
    - 直接dfs返回的就是递归完成后整个路径上的面积, 一直累加即可, 最后res通过max来判断最大

- **不同的岛屿**
  - [1905-统计子岛屿](./countSubIslands.cpp), 与封闭岛屿类似, 一旦题目将岛屿进行了分类, 就可以用逆向思维排除法来尝试
    - 即: 一个岛屿, 要么是子岛屿, 要么不是子岛屿; 那把所有的非子岛屿淹掉 (排除掉), 剩下的就都是子岛屿了
    - 判断非子岛屿的方法: 有的格子grid2中**是陆地**, 但是grid1中**却是海洋**; 因此只要这个条件满足时, 就可以先把grid2中对应的非子岛屿全淹掉
    - 剩下再来一个双for循环统计剩余的岛屿, 就都是子岛屿了
  - [694-不同的岛屿数量](./numDistinctIslands.cpp), 明确不同的岛屿, 也就是寻找一种序列化方法把所有的岛屿找见, 最后用hashset去重返回
    - 序列化: 在遍历方向唯一且确定时, 遍历一个岛屿的顺序就可以看作是一种序列化方案. 那么形状相同的岛屿**遍历的次序**必然也是一样的
    - 因此要为每个方向标记一个数, 并且在递归撤出时, 也要在后序位置记录撤离的顺序
      - 因为**先撤离**还是**先遍历**, 其实是两种不同的选择, 尽管在整个递归撤离后遍历的数字是一样的, 但岛屿的形状其实是不同的
      - 如: 先下, 再右, 撤销右, 撤销下 与 先下, 撤销下, 再右, 撤销右 就是两个形状不同的岛屿
    - 此时dfs的签名为`dfs(vector<vector<int>>& grid, int x, int y, string& curr, int dir)`, curr即为记录遍历次序的string, dir即为遍历方向对应的数字

- [37-解数独](./solveSudoku.cpp), 通过回溯算法进行暴搜, 暴搜的顺序先列再行
  - 这里的暴搜就是直接尝试1-9字符, 只不过每次尝试前都需要先判断下当前选择是否为valid的
  - 不要忘记尝试填一个位置时, 对3x3的周围区域也进行验证


## BFS框架

- [111-二叉树的最小深度](./minDepthTree.cpp): BFS的模板题, 当遍历到一个节点的左右孩子均为空时说明遍历到头了
  - 这题的难点在于想到用BFS来做, 诚然用DFS其实可以, 但时间复杂度会高很多
- [752-打开转盘锁](./openLock2.cpp): 由于每次只能拨动1个数字的1位, 因此对于4位长的锁而言, 每一步相当于有8种可能性, 也就是8个分支
  - 那么这样用BFS去搜索就行, 因为起点固定为0000, 那么BFS找出来的一定是最短的步骤 (本质上就相当于按层暴搜)
  - 每次BFS框架里的for循环在push新的元素时, 其实就是push每一位对应的 +1/-1后的结果 (注意用一个visited来避免重复push)
  - 关键在于将拨动锁找答案的过程抽象为BFS的路径搜索过程 **(选择与节点)**

> 双向BFS: 当知道最终搜索的目标时, 可以同时也从target开始, 即双向BFS, 这样只要两个BFS相交, 就说明路径是通的. 从Big O的角度来看, 都是一样的, 只是实现效率上的差异
>  - 而且这里的双向扩散并不是同时扩散, 而是在每次while循环的时候, 两个queue轮流扩散, 所以本质上的复杂度是相同的

- [773-滑动谜题](./slidingPuzzle2.cpp), 与开密码锁的题很类似, 同样是将每一步的状态抽象
  - 所谓的搜索其实就是将0位和周围数字都交换一次, 形成的新状态加到queue中
  - 同样需要visited, 以及需要一个能直接找到索引的邻居位置的查找表, 从而能直接生成每个状态的相邻状态
  - 这里的查找表与上一题的add/plus one都是BFS的**工具函数**, 除此以外二者的架构和逻辑都完全一致, 只是具体的state不同



## 数学运算技巧

- [384-打乱数组](./shuffleVector.cpp), **洗牌算法**让每个位置的元素都正确的被置乱
  - 第一个位置有n种可以选, 第二个位置有n-1种, 一共一起就有 n! 种, 跟全排列一样
  - 用一个for循环实现, 每次在剩下的位置里随机选一个位置, 与当前元素交换.

- [382-链表随机节点](./RandomLinkedList.cpp), **水池抽样算法**, 即在不重复遍历的条件下, 对每个元素都能等概率的选中
  - 这里返回1个随机元素和返回k个随机元素都是类似的, 本质上都是在遍历每个元素时, 随机一个数来决定是否选中当前元素
  - 水池抽样算法里: 每个数选中的概率应为 1/i, 可以用 rand() % i == 0 来生成 1/i 的这个概率
  - 对于返回1个元素, 就都遍历一遍用一个随机变量 (*条件: rand() % i == 0*); 对于返回k个元素, 先用头k个元素放到数组里, 再遍历随机选中 (*条件: j = rand() % i, j < k*)
  - 相似题目: [398-随机数索引](./RandomIndex.cpp), 这道题并不是在所有元素中抽样, 而是在等于target的元素中抽样
    - 所以基本代码框架不变, 循环中维护一个count来记录当前target有多少个, 然后 rand() % count 来实现水塘抽样
    - 但是现在leetcode 上面水塘抽样会超时, 只能直接暴力用哈希表来做了

### 位运算

- [191-位1的个数](./hammingWeight4.cpp): 核心就是 **n & (n - 1)**, 这样一定可以把n最后的那个1跟0作与运算之后消掉 (因为-1的效果)
  - 所以只需要在n != 0的条件下, 看 n & (n - 1) 能执行多少次即可
  - 类似题目: [231-2 的幂](./isPowerOfTwo2.cpp): 2的幂有两个条件: 大于0, 且1只出现过一次
    - 一行解决问题: (n > 0) && ((n & (n - 1)) == 0)

- [136-只出现一次的数字](./singleNumberI.cpp): 异或的运用, 题目就差把异或写脸上了
  - 进阶: [268-丢失的数字](./missingNumber2.cpp), 因为数组元素的范围就是 0-n, 这恰好跟索引的范围相同 (索引式0-n-1)
    - 那么, 就可以借助索引的数字来**凑对异或**, 最终异或后的就是剩下的那个落单的
    - `res ^= i; res ^= nums[i]`

- [645-错误的集合](./findErrorNums2.cpp): 相比之前的题, 不仅需要找出缺失的, 还得找出重复的; 同样是利用索引本身就是数组元素内容的特点, 只不过这里不是用异或, 而是用索引与元素之间的**映射**
  - 核心思想: 按顺序遍历, **将遇到的元素所代表的索引值上的元素置负**, 一旦遇到已经成为负的元素, 那就找到了重复元素; 再遍历一次如果遇到了索引值元素为正的, 那就找到了缺失元素
  - 本质就是: `nums[nums[i]] *= -1` (只不过这里`nums[i]`对应的索引需要-1处理下, 以及套个abs防止错误)
  - 这个处理方法很妙, 将元素和索引映射起来之后, 用元素本身来反映遍历情况, 用正负来反映访问情况, 理解了之后就容易写出来了

### 阶乘

- [172-阶乘后的零](./trailingZeroes.cpp): 阶乘里能分出几个2x5 => 只要有5, 必然有2 => 阶乘里能分出几个因子5 => 分情况逐步讨论
  - 贡献1个因子5的有几个数, 2个因子5的有几个数, ....
  - 在实现时, 可以用循环来覆盖任意n的情况 `divisor = 5; while(){divisor *= 5}`
  - 这个题的思路以及实现都有很强的技巧性

- [793-阶乘函数后 K 个零](./preimageSizeFZF.cpp): 没想到可以用二分来搜索; 在172的基础上, 可以用二分搜索满足条件的左边界和右边界, 再将边界相减
  - 因为n作为自然数递增, 所以符合条件的n一定是连续的若干个数 (这一点观察很关键)
  - 之后就是用二分的模板, 来对 `[0, long_max)`区间搜索, 以找到 `trailingZeroes(n) == k` 的左右边界; 对于左边界, 就需要right = mid 来不断紧缩; 对于右边界, 需要left = mid + 1紧缩; 
    - 这里需要仔细重温下二分的模板写法, 以及搜索时的区间端点开闭情况


- **找素数**: [204-计数质数](./countPrimes2.cpp): 用筛法实现, 注意筛的时候打表可以从 `i * i` 开始, 而非 `i * 2`, 因为小于 i * i 必然都已经计算过了

- [372-超级次方](./superPow2.cpp): 通过数学的性质进行递归 $a^{24}=a^{4}\cdot a^{20}$, 即每次都算最后一位的, 逐步累乘
  - 这样就相当于分成两支来递归: `mypow(a, last_bit) * mypow(superPow(a, b), 10);`, 这里就是等价于继续往下走
  - 在实现mypow时, 注意不要溢出; 而在superpow里, 注意结束的条件

- [241-为运算表达式设计优先级](./diffWaysToCompute.cpp): 不要陷入表达式求解的窠臼里, 从最开始无嵌套括号的形式考虑, 逐层深入, 最后到两个数的运算
  - 加括号的本质就是在对运算进行分隔, 而括号嵌套本质上的逻辑就是在递归, 因此只需要想好一层的递归加括号, 以及运算该怎么实现即可
  - 为了枚举出所有的结果, 当前函数在递归的意义上有两个作用: 分 (即根据运算符左右分隔, 并调用下一层递归) 和 治 (将递归返回的结果进行单运算符的计算)
  - 注意利用好返回结果也是 `vector<int>` , 因为下层递归也恰好返回的是所有结果, 所以在 "治" 这一步时, 直接遍历返回的两个`vector`直接运算即可

- **区间重叠问题**: [1288-删除被覆盖区间](./removeCoveredIntervals.cpp): 题目有隐含条件是, 如果一个区间能被若干个区间合并后的区间覆盖, 也算是覆盖区间
  - 因此, 除了要将区间数组进行排序外, 在遍历过程中需要维护一个当前合并区间, 以全面地筛选出覆盖的区间
  - 在对区间排序时, 按照左端点升序, 右端点降序来, 有点类似于贪心的策略
  - 排序完成后, 根据两个区间之间的 重叠, 覆盖, 完全不相交 这三种情况来动态维护一个当前用于判定覆盖的大区间

- [56-合并区间](./mergeInterval.cpp): 要实现的功能被上一题包含了, 但如果追求简单效率的话
  - 只需要区间左端点排序, 然后动态对比右端点
  - 只要右端点小于下一个区间的左端点, 然后就可以比较当前右端点和下一个区间的右端点谁大; 否则的话, 更新当前的这个左右端点
  - `last[1] = max(last[1], curr[1])`

- [986-区间列表的交集](./intervalIntersection.cpp): 核心在于两个地方: 如何知道两个相交区间的相交部分 与 如何判断什么时候该去判定下一个区间
  - 对于第一个问题, 如果两个区间真的相交 (`a[0] <= b[1] && a[1] >= b[0]`), 那么相交部分就是 左右端点的 max 与 min之间的部分 (`{max(a[0], b[0]), min(a[1], b[1])}`);
  - 对于第二个问题: 以firstList为基准的话, 当secondList区间的右端点超过了 (>) firstList里的左端点时, 就说明需要移动了
  - 整个解法的结构就是非常简洁的一个循环, 然后分别维护两个指针来游走两个列表, 进行区间相交情况的判定


- [659-分割数组为连续子序列](./isPossibleSplitSequence.cpp): 既然题目指出了是连续三个数字的子序列, 那我们就用连续的三个freq来判定是否为连续序列; 
  - freq表示每个数字出现的频率, next表示如果有序列的话, 下一个期望数字的频率
  - 充分利用题目已经排序好这一点, 可以直接从数字出现的频率来着手

- [969-煎饼排序](./pancakeSort2.cpp): **先明确找一个最大元素**: 先找到当前最大的元素, 然后把截止到最大元素的前若干元素反转, 然后再把整个数组反转, 这样最大的元素就在最后了
  - 之后可以递归, 也可以for循环迭代, 对剩下的元素依次进行相同操作即可
  - 注意这只是一种排序法, 并不是最优的

- [43-字符串相乘](./stringMultiply.cpp): 依次执行两位数乘法, 然后将运算过程中产生的进位, 以及填到结果中的个位数与之前的结果进行更新
  - 这需要每次运算时, 都知道当前两位运算结果对应到最终结果里是第几位 (其实就是第`i + j`与`i + j + 1`位)
    - `res[p1] += sum / 10;  res[p2] = sum % 10;`
  - 最后还需要特别处理结果中开头未被填入的位置

- [224-基本计算器](./calculate.cpp): 从左往右遍历, 括号部分可以用**递归**来处理
  - 对括号的处理: 遇到括号就执行递归, 化整为零, 只需考虑无括号时的运算即可
  - 对普通运算的处理: 维护一个**双端队列**, 在遇到运算符时, 实际上计算的是上一个运算符 (需要一个额外的`sign`变量) 的结果.
  - 对数字的运算: 本质上是从左到右的`char`to`int`的十进制转换, 需要维护一个单独的`num`变量来记录
  - 最后, 保证双端队列中的元素都是可以通过累加直接得到的预计算后的值, 这样直接`accumulate`即可

- [42-接雨水](./trapWater.cpp): 本质上就是 `res += min(lmax, rmax) - height[i]` 只不过这里如何确定`lmax`与`rmax`有不同的方法:
  - 暴力解: 在每个位置i时, 直接向前或向后遍历, 确认最高的值 (但这样复杂度是$N^{2}$)
  - 备忘录: 提前遍历好每个位置i对应的`lmax`, `rmax`, 然后遍历时直接累加 (比较好理解的解)
  - 双指针: 其实思考整个过程, 有些位置上的`lmax`或`rmax`其实是被浪费掉的, 所以完全可以从两端遍历, 然后只要当前位置的max是更小的那一个, 也就说明目前为止雨水都是能接住的. 如果当前的max更大了, 就需要从另外一边 (也就是更小的) 开始接
  - [11-盛最多水的容器](./trapWaterMax.cpp): 这道题的特点是每个柱子没有宽度, 即相当于一些细线, 然后每个细线之间的宽度是1
    - 这样一来, 盛水的容量为`(right-left) * (min(height[left], height[right]))`, 也就是两个max之间的矩形
    - 基本的思想依旧是用了双指针的移动策略, 在循环时更新最大值即可. 


- **括号闭合问题**
  - [20-有效的括号](./isParenthesesValid.cpp): 注意题目要求是, 三种括号必须各自先闭合, 因此在栈里需要判定栈顶与当前右括号是否组对, 如果不对, 直接`false`
    - 可以写一个`leftOf`函数来返回某个右括号对应的左括号是什么
  - [921-使括号有效的最少添加](./minAddToMakeValid.cpp): 以左括号为基准, 维护两个变量`left_need, right_need`, 分别记录左右括号需要的数量. 
    - 以左括号为基准时, 主要变化`right_need`, 当右括号太多时, 需要更改`left_need`来增加左括号保证平衡
    - 当然也可以以右括号为基准, 但遍历时应从后往前遍历, 否则会导致错误的判断了要插入右括号的数量
  - [1541-平衡括号字符串的最少插入次数](./minInsertionsWith2Right.cpp): 与921很像, 但是需要增加两个右括号
    - 此时注意两个变量含义有所区分, 不再是左右括号的需要数量了;
    - 在插入时, 注意右括号的需求是否有溢出, 或者奇偶不匹配的情况


- [391-完美矩形](./isRectangleCover2.cpp): 核心是要明确究竟 "完美矩形" 等价于 什么样的条件:
  - (1) 最终大矩形的**面积**必须等于原有小矩形面积相加; (2) 最终形成的图案只能有4个**拐角点**; (3) 这4个顶点还得恰好是理论上的边界顶点
  - 面积维度: 找出理论边界顶点, 然后计算并比较面积即可
  - 顶点维度: 出现了奇数次的点, 最终会成为完整大图形里的拐角点, 偶数次则不会
    - 顶点维度是比较不容易想到的, 而且还得要求最后的拐角点就是理论上的边界顶点.


- [855-考场就座](./ExamRoom.cpp): 对平衡二叉树 (set in C++) 的熟练应用
  - 选取一种数据结构使之能: 随时找到最大的元素 (动态有序), 还能插入和删除任意值 (二叉堆只能删除最大值) => 平衡二叉树
  - 且还需要重载其 **Comparator**, 以满足元素相等时按另外的维度 (位置索引) 来排序的特点
    - 在本题中, 排序的首要依据不是座位线段端点的长度, 而应该是**线段中点到端点**的长度 (即可能会被安排的座位)
  - 而在`seat`中, 本质就是取出当前座位线段的最大值, 然后分割成左右两部分再加到树里
  - `leave`中, 也是根据座位索引得到存储在树中的左右端点, 移出分线段后加入合并后的线段


- [392-判断子序列](./isStringSubsequence.cpp): 本质上就是将母串的字符索引化, 然后用二分来实现空间换时间
  - 核心思想很简单, 先用一个`vector<vector<>>`来存好母串每种字符的索引位置, 然后对比子串. 在寻找子串时用二分来快速找到索引
  - 类似题目: [792-匹配子序列的单词数](./numMatchingSubseq.cpp), 从匹配一个变成了匹配多个, 注意索引的更新 以及 break 跳出后的判断 (`cnt`不是无条件累加!)